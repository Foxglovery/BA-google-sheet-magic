function onEdit(e) {
    ...
    const orQtySoldCol = 3; // C: Quantity Sold
    const orBatchCol   = 4; // D: Batch ID dropdown
    const orStatusCol  = 5; // E: Status
    ...
    if (sh.getName() === sheetName) {
      ...
      if (col === productCol && e.value) {
        ...
        // 1a) clear out any old dropdowns in column D
        sh.getRange(2, dcCol, sh.getLastRow() - 1).clearDataValidations();
  
        // 2) build & apply the new DC dropdown for this row
        const rawProduct = e.value.toString();
        const doseCode   = (rawProduct.match(/(D8|D9|THCO|FS|CAF)/) || [])[1];
        if (doseCode) {
          const distSh = ss.getSheetByName(distShName);
          if (distSh) {
            const last = distSh.getLastRow();
            // pull BatchCode (col B) & Cannabinoid (col C)
            const distData = distSh
              .getRange(2, 2, last - 1, 2)
              .getDisplayValues();
            const opts = distData
      ...
      if (col === dcCol && e.value) {
        ...
        // extract only the batchCode (strip off “-D9” etc)
  ...
  if (sh.getName() === ordersSheet) {
    // a) PRODUCT pick → clear stale, then populate Batch ID dropdown in D
    if (col === orProductCol && e.value) {
      // nuke all old validations in column D
      sh.getRange(2, orBatchCol, sh.getLastRow() - 1).clearDataValidations();
  
      // now collect your 3 most recent batch codes…
      const rawProduct = e.value.toString();
      ...
      // apply the new dropdown to this single cell
    ...
    // b) STATUS = Complete → subtract Quantity Sold (col C) from inventory
